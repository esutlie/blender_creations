...
class HeadMount2:
    def __init__(self, clearance_modifier=0, clearance_modifier2=0, DOVE_OUTER_WIDTH=2.7, holder_screw_radius=1):
        self.objects = []

        # This: pixel settings
        self.probe_y_post_modifier = 0
        self.probe_x_post_modifier = 0 + clearance_modifier2
        ...
        self.DOVE_OUTER_WIDTH = 2.8 + clearance_modifier  # 2.8, 2.78 are too large, 2.7 is too small
        self.DOVE_DEPTH = .75
        ...
        self.SCREWHEAD_LOWER_RADIUS = 1.2 + clearance_modifier
        self.SCREWHEAD_DEPTH = 1.08 + clearance_modifier
        self.SCREWHEAD_UPPER_RADIUS = self.SCREWHEAD_DEPTH + self.SCREWHEAD_LOWER_RADIUS + .1
        self.SCREW_THREAD_RADIUS = 1.09 + clearance_modifier / 2
        self.DRIVER_WIDTH = 3
        self.SCREW_LENGTH = 16

        # clearance for cap should .11
        # clearance for cover should be .15
        self.cap_clearance = .15 + clearance_modifier
        self.dove2_clearance = .24 + clearance_modifier
        # self.cap_clearance = .15  # was .13
        # self.cover_clearance = .18  # was .16
        self.cover_clearance = .18 + clearance_modifier

        self.protrusion = 5
        self.hex_radius = 2.49 + clearance_modifier

        self.lip_height = 1
        self.lip_outer_radius = .9
        self.lip_inner_radius = .65
        ...
        self.holder_clearance = .15 + clearance_modifier
        ...
        self.grinder_post_separation = 4.4
        ...
        self.stand_clearance = .35 + clearance_modifier
        self.cleaner_clearance = .4 + clearance_modifier
        self.base_width = 30
        ...
        self.hex_depth = 2.08 + clearance_modifier
    ...
    def holder(self, shield=False, type='holder', name=None):
        if not name:
            name = type
        ...
        if type == 'holder' or type == 'grinder':
            ...
            if type != 'grinder':
                ...
                rotate(pi / 6, 'Z')
                select_verts(hex_cut, INF_NEG, INF_POS, INF)
                resize([2.1, 1, 1])
                mode('OBJECT')
            ...
            select_verts(cutter, [-100, 100], [xyz_location[1], 100], [-100, 100])
            translate([0, self.probe_y_post_modifier, 0])
            ...
            mode('OBJECT')
            translate([-self.probe_x_post_modifier, 0, 0])
        ...
        if type == 'stopper':
            cyl_filler = add_cylinder(self.lip_inner_radius - self.holder_clearance, self.lip_height / 2, 32,
                                      'cyl_filler')
    ...
    def cap(self, shield=False, name='cap'):
        ...
        outer_lip = add_cylinder(self.lip_outer_radius, self.lip_height, 100, 'outer_lip')
        translate((self.PIXEL_X_OFFSET, 0, -self.lip_height / 2 + .001))
        oval_amount = .3
        resize([1 + oval_amount * self.lip_inner_radius / self.lip_outer_radius, 1, 1])
        inner_lip = add_cylinder(self.lip_inner_radius, self.lip_height, 100, 'inner_lip')
        ...
        translate([self.PIXEL_X_OFFSET, self.holder_xyz[1], 0])
        for side in [-1, 2]:
            translate([side * (self.cap_size[0] + self.cap_window_width - .1) / 2, 0, 0])
    ...
    def stopper(self, shield=False, name='stopper'):
        # This makes the piece that permanently attaches to the pixel
        holder_location = [0, self.holder_size[1] / 2, self.holder_size[2] / 2]
        stopper = add_cube(self.holder_size, holder_location, name)
        for x in [[0, 100], [-100, 0]]:
    ...
    def tube(self, play=False):
        ...
        # This makes the tube
        name = 'play' if play else 'tube'
        tube = add_cylinder(radius + thickness, length, vertices, name)
        ...
        translate((0, 0, self.head_fix_size[2] + self.headfix_bar_z))

        if play:
            delete([tube_cut, slit])
            return tube
        else:
            # This cuts out the headfix piece
            headfix_cut_size = [self.head_fix_size[0] + 2 * self.headfix_clearance,
                                self.head_fix_size[1] + self.headfix_clearance + .02,
                                self.head_fix_size[2] + 1.8 * self.headfix_clearance]
            headfix_cut_location = [0, self.holder_size[1] / 2 + self.holder_xyz[1] + self.headfix_clearance / 2,
                                    self.head_fix_size[2] / 2 + self.headfix_bar_z]
            ...
            headfix_translate = (0, self.holder_size[1] / 2 + self.holder_xyz[1],
                                 (self.head_fix_size[2] + 2 * self.headfix_thickness) / 2
                                 - self.headfix_thickness + self.headfix_bar_z)
            for side in [1, -1]:
                distance = 2.3
                hex_location = [side * (self.head_fix_size[0] / 2 + distance),
                                -self.head_fix_size[1] / 2 - distance + self.headfix_clearance,
                                self.head_fix_size[2] / 2 + distance]
                hex_cut = add_cylinder(self.hex_radius, self.hex_depth, 6, 'hex_cut')
                mode('OBJECT')
    ...
    def stand(self, test=False):
        ...
        # This adds guides for the holder
        thickness = self.cap_wall_thickness
        guide_size = [self.stand_size[0] - .1, thickness, self.cap_size[2] - self.cap_top_extention]
        loc = [0, -thickness / 2 - self.holder_y_extension - self.PIXEL_PROBE_Y - self.cap_clearance, guide_size[2] / 2]
        guide = add_cube(guide_size, [0, 0, 0], 'guide')
        translate(loc)
        center_cut_size = [self.holder_size[0] - self.holder_wall_thickness * 2, thickness + 1, self.cap_size[2] + 1]
        center_cut = add_cube(center_cut_size, loc, 'center_cut')
        boolean_modifier(guide, center_cut)
        select_verts(guide, [-center_cut_size[0] / 2, center_cut_size[0] / 2], INF, INF_NEG)
        bevel(self.holder_wall_thickness + self.cap_wall_thickness + self.cover_thickness)
        activate([center_cut])
        translate([center_cut_size[0] / 2 + self.cap_size[0] / 2, 0, 6])
        boolean_modifier(guide, center_cut)
        activate([center_cut])
        translate([-center_cut_size[0] - self.cap_size[0], 0, 0])
        boolean_modifier(guide, center_cut)
        boolean_modifier(stand, guide, modifier='UNION')

        delete([hole, clear, base, clean_cut, handle_cut, cap_dovetail, screw_clear, screw_cut, center_cut, guide])
    ...
    def headfix(self):
        headfix_size = [self.bridge_width + 2 * self.bridge_thickness,
                        self.head_fix_size[1] + 2 * self.headfix_thickness,
                        self.head_fix_size[2] + 2 * self.headfix_thickness]
        ...
        cut2_size = [self.head_fix_size[0] + 2 * self.headfix_clearance,
                     self.head_fix_size[1] + self.headfix_thickness + self.headfix_clearance + .002,
                     self.head_fix_size[2] + 2 * self.headfix_clearance]
        cut2_location = [0, -cut2_size[1] / 2, 0]
        cut2 = add_cube(cut2_size, cut2_location, 'cut2')
        bpy.ops.transform.translate(value=(0, self.head_fix_size[1] / 2 + self.headfix_clearance, 0))
...
class Chamber:
    def __init__(self, overrider):
        ...
        self.back_dove_angle = 51 * pi / 180
        self.thickness = .7
    ...
    def frame(self):
        ...
        select_verts(obj_frame, INF, [-100, self.thickness / 2], INF_NEG)
        extrude_move([0, 0, -self.h_locking])
        select_verts(obj_frame, INF, [-100, -self.thickness / 2], [-100, -self.h_locking])
    ...
    def base(self, flat_side=False):
        ...
        # This flattens a side if necessary
        if flat_side:
            flat_cut = add_cube([1.97, 6, 10], (0, 0, 0), 'flat_cut')
            rotate(-(pi - self.angle_port) / 2, 'Z')
            translate([self.width_port / 2, 0, -5])
            boolean_modifier(obj_base, flat_cut)
            delete([flat_cut])
    ...
    def port(self, blank=False):
        ...
        # for x in [[window_width / 2,window_width / 2],[-window_width / 2,-window_width / 2]]:
        #     select_verts(obj_port, x, [0, 0], [port_z[1], port_z[2]])
        #     bevel(.2)
        ...
        ir_pos = [(port_x[1] - ir_x_depth - .05, ir_y_depth, port_z[1] + port_ir_height),
                  (port_x[2] - ir_x_depth, port_y[1] + ir_y_depth_lick, tube_height - .07),
                  (-port_x[1] + ir_x_depth + .05, ir_y_depth, port_z[1] + port_ir_height),
                  (-port_x[2] + ir_x_depth, port_y[1] + ir_y_depth_lick, tube_height - .07)]
        ir1 = self.build_ir()
        ...
        cube_cut = add_cube([window_width - .1, port_y[2] - port_y[1], port_y[2] - port_y[1]], [0, 0, 0], 'cube_cut')
        rotate(pi / 4, 'X')
        bpy.ops.object.transform_apply(location=True, rotation=True, scale=True)
        resize([1, 1, .6])
        translate([0, port_y[1] - .1, LED_z - .03])
        boolean_modifier(obj_port, cube_cut)
        delete([cube_cut])
    ...
    def tictac(self):
        inside_point = self.width_port / 2
        theta = (pi - self.angle_port) / 2
        offset = self.c_port / (2 * sin(theta))
        port_width = inside_point * 2 - offset
        inner_width = 2 * port_width * sin(theta)
        thickness = self.thickness
        height = self.h_base + self.h_bottom + self.h_locking - self.clearance_base

        # This builds the initial shape
        obj_tictac = add_cube([inner_width, thickness, height], (0, 0, 0), 'tictac')
        select_verts(obj_tictac, INF, INF_POS, INF)
        resize([1 + 2 * thickness * tan(pi - 2 * theta) / inner_width, 1, 1])
        extrude_move([0, 1, 0])
        resize([.7, 1, 1])
        mode('OBJECT')

        # This cuts out the nose poke
        LED_diameter = .51
        nose_poke_resize = 1.5
        led_cyl = add_cylinder(LED_diameter / 2, thickness, 32, 'led_cyl')
        rotate(pi / 2, 'X')
        translate([0, 1.001, 0])
        select_verts(led_cyl, INF, INF, INF_NEG)
        extrude_move([0, -1.002, 0])
        resize([nose_poke_resize, 1, nose_poke_resize])
        mode('OBJECT')

        # This adds the IR cutters
        ir_pos = [(-LED_diameter * 1.1, -thickness / 4 - .1, 0),
                  (LED_diameter * 1.1, -thickness / 4 - .1, 0)]
        ir1 = self.build_ir(beam_diameter=.15)
        bpy.ops.transform.translate(value=ir_pos[0])
        ir2 = self.build_ir(beam_diameter=.15, side='left')
        bpy.ops.transform.translate(value=ir_pos[1])
        bpy.ops.transform.rotate(value=pi, orient_axis='Y')

        activate([led_cyl, ir1, ir2])
        translate([0, 0, -1.3])
        rotate(pi / 4, 'Y')

        separation = 1.05
        for [x, z] in product([-1, 0, 1], repeat=2):
            activate([led_cyl, ir1, ir2])
            translate([x * separation, 0, z * separation])
            boolean_modifier(obj_tictac, led_cyl)
            [boolean_modifier(obj_tictac, cutter) for cutter in [ir1, ir2]]
            activate([led_cyl, ir1, ir2])
            translate([-x * separation, 0, -z * separation])

        delete([led_cyl, ir1, ir2])

        activate([obj_tictac])
        translate([inside_point, 0, 0])
        rotate(pi / 2 - theta, 'Z')
        shift = port_width * cos(theta) - thickness / 2
        translate([-shift * cos(theta), -shift * sin(theta), -height / 2 - self.clearance_base])

        # This makes the piece to cut away the front of the locking mechanism
        front_cut = add_cube([self.width_port, self.thickness, self.h_locking + self.clearance_base], [0, 0, 0],
                             'front_cut')
        translate([0, 0, -(self.h_locking + self.clearance_base) / 2 + .001])
        resize([1 + thickness * tan(pi / 2 - theta) / self.width_port, 1, 1])
        select_verts(front_cut, INF_NEG, INF, INF)
        extrude_move([-1, 0, 0])
        select_verts(front_cut, INF, INF_NEG, INF)
        extrude_move([0, -1, 0])
        select_verts(front_cut, INF_POS, INF, INF)
        extrude_move([4, -4, 0])
        mode('OBJECT')
        translate([self.clearance_base / 2 * cos(theta), self.clearance_base / 2 * sin(theta), 0])
        bpy.ops.object.duplicate()
        front_cut2 = bpy.context.active_object
        bpy.context.active_object.name = 'front_cut2'
        d = thickness / cos(pi / 2 - theta) / 2 + 2 * self.clearance_base
        translate([-d * cos(theta), -d * sin(theta), 0])
        resize([1, 1, 1.1])
        front_cut3 = add_cube([15, 3, 10], (0, 0, 0), 'front_cut3')
        translate([self.width_port / 2, 0, -(self.h_locking + self.clearance_base) / 2 + .001])
        translate([-2.3 * cos(theta), -2.3 * sin(theta), 0])
        rotate(pi / 8, 'X')
        rotate(pi / 2 - theta, 'Z')

        boolean_modifier(front_cut2, front_cut3)
        boolean_modifier(front_cut, front_cut2)
        boolean_modifier(obj_tictac, front_cut)
        delete([front_cut, front_cut2, front_cut3])

        # This makes the piece to cutaway the back of the locking mechanism
        back_cut = add_cube([self.width_port, 2, self.h_locking], (0, 0, 0), 'back_cut')
        translate([0, 1 + self.thickness, -self.h_locking / 2])
        resize([1 + 2 * thickness * tan(pi / 2 - theta) / self.width_port, 1, 1])
        select_verts(back_cut, INF_NEG, INF, INF)
        extrude_move([-1, 0, 0])
        select_verts(back_cut, INF_POS, INF, INF)
        extrude_move([4, -4, 0])
        boolean_modifier(obj_tictac, back_cut)
        delete([back_cut])
    ...
    def blank_tri(self):
        size = [self.width_port, self.width_port / 3, self.h_bottom - self.c_port]
        loc = [0, -size[1] / 2, -size[2] / 2 - self.h_locking - self.c_port]
        tri = add_cube(size, (0, 0, 0), 'tri')
        translate(loc)
        select_verts(tri, INF, INF_NEG, INF)
        resize([.1, 1, 1])
        select_verts(tri, INF, INF_NEG, INF_POS)
    ...
    def cam_attach(self):
        ...
        boolean_modifier(cam_attach, screw_cut, fast_solver=True)
        delete([screw_cut])
    ...
    def sol_holder(self, test=False, num=2):
        ...
        base = add_cube(base_size, [0, 0, 0], 'sol_base')
        translate((0, 0, base_size[2] / 2))
        select_verts(base, INF, INF, INF_NEG)
        bevel(base_size[2] * .9, segments=5)
        mode('OBJECT')
        holder = add_cube(holder_size, [0, 0, 0], 'holder')
        translate([0, 0, holder_size[2] / 2 + base_size[2] - .01])
        holder_cut = add_cube([holder_size[0] - 2 * leg_width, holder_size[1] + 1, holder_size[2]], (0, 0, 0),
                              'holder_cut')
        translate([0, 0, holder_size[2] / 2 + base_size[2] - bridge_height])
        boolean_modifier(holder, holder_cut)
        boolean_modifier(holder, base, 'UNION')
        ...
        extrude_move([0, 0, stand_top_depth - stand_size[1]])
        resize([1, stand_top_depth / stand_size[1], 1])
        translate([0, -stand_top_depth / 2 + stand_size[1] / 2, 0])
        extrude_move([0, 0, .25])
        mode('OBJECT')
        tube = add_cylinder(tube_diameter / 2, tube_height, 50, 'tube')
        translate(
        ...
        select_verts(screw_hole, INF, INF, INF_POS)
        extrude_move([0, 0, 1])
        ...
        if test:
            ...
            test_block = add_cube(test_block_size, [0, 0, 0], 'test_block')
            translate([stand_loc[0], 0, test_block_size[2] / 2 + base_size[2] + holder_size[2] - bridge_height - .01])
